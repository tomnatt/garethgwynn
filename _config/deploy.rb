...
task :audio do
  ...
  system "rsync -az -e ssh #{fetch(:audio_source)}/listenagainst/ #{fetch(:user)}@#{fetch(:domain)}:#{path}/listenagainst/"
  system "rsync -az -e ssh #{fetch(:audio_source)}/otherdemos/ #{fetch(:user)}@#{fetch(:domain)}:#{path}/otherdemos/"
end
